...
impl FlatCamApp {
    /// Build a `FlatCamApp` with the same initial defaults `main()` applies, and
    /// install the UI fonts on the given context. Shared by the desktop binary
    /// and the headless `screenshot` binary so both start from an identical app.
    pub fn boot(ctx: &egui::Context) -> Self {
...
impl FlatCamApp {
    /// Build the entire UI from `ctx`. Split out of [`eframe::App::update`] so a
    /// headless harness (the `screenshot` binary) can drive the same UI without
    /// an [`eframe::Frame`].
